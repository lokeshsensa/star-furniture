...
export const IntroLogo: React.FC<IntroLogoProps> = ({ onComplete }) => {
  ...
  useGSAP(
    () => {
      ...
      // Ambient center white radial glow
      if (glowRef.current) {
        masterTl.to(
          glowRef.current,
          {
      ...
      // Logo reveal: scale 0.7 -> 1, opacity 0 -> 1, blur 16px -> 0
      if (logoRef.current) {
        masterTl.to(
          logoRef.current,
          {
      ...
      // Subtle ambient floating wave motion
  ...
  return (
    ...
    >
      {/* LEFT FLUID LIQUID BLUE WAVE SWOOSH */}
      <div
        ref={leftWaveRef}
      ...
      {/* RIGHT FLUID LIQUID GREEN WAVE SWOOSH */}
      <div
        ref={rightWaveRef}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-4 max-w-5xl w-full text-center">
        <div className="w-[85%] sm:w-[78%] md:w-[70%] lg:w-[64%] max-w-[850px] flex justify-center">
          <img
            ...
            className="w-full h-auto object-contain drop-shadow-[0_20px_40px_rgba(6,91,182,0.15)]"
          />
        </div>
      </div>
